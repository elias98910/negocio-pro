    ...
    with get_connection() as conn:
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS productos (
                ...
                categoria TEXT, stock REAL DEFAULT 0, stock_minimo REAL DEFAULT 0,
                costo_unitario REAL DEFAULT 0, precio_venta REAL DEFAULT 0,
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS venta_detalle (
                id SERIAL PRIMARY KEY, venta_id INTEGER REFERENCES ventas(id),
                producto_id INTEGER REFERENCES productos(id), cantidad REAL NOT NULL,
                precio_unitario REAL NOT NULL, costo_unitario REAL NOT NULL, subtotal REAL NOT NULL
            )
        """)
        c.execute("""
        ...
        c.execute("""
            CREATE TABLE IF NOT EXISTS compra_detalle (
                id SERIAL PRIMARY KEY, compra_id INTEGER REFERENCES compras(id),
                producto_id INTEGER REFERENCES productos(id), cantidad REAL NOT NULL,
...
def pagina_dashboard():
    ...
    with get_connection() as conn:
        stock_bajo = pd.read_sql_query(
            "SELECT nombre, stock, stock_minimo, unidad FROM productos WHERE activo=1 AND stock <= stock_minimo AND stock_minimo > 0", conn
        )
...
def pagina_inventario():
    ...
    with tab4:
        if df.empty:
            ...
            prod_id = st.selectbox("Selecciona producto a eliminar", options=df["id"].tolist(),
    ...
    if st.button("➕ Agregar al carrito"):
        if cantidad > p["stock"]:
            ...
            st.session_state.carrito.append({
                "producto_id": int(prod_id), "nombre": p["nombre"], "cantidad": cantidad,
                "precio": precio, "costo": float(p["costo_unitario"]), "subtotal": cantidad * precio
            })
    ...
    if st.session_state.carrito:
        ...
        if st.button("✅ Confirmar Venta", type="primary"):
            try:
                ...
                with get_connection() as conn:
                    c = conn.cursor()
                    c.execute("INSERT INTO ventas (cliente, total, descuento, metodo_pago) VALUES (%s, %s, %s, %s) RETURNING id",
...
def pagina_compras():
    ...
    costo = st.number_input("Costo unitario de esta compra", min_value=0.0,
                            value=float(df[df["id"]==prod_id]["costo_unitario"].values[0]), step=0.01, key="c_costo")
    
    if st.button("➕ Agregar"):
        nombre = df[df["id"]==prod_id]["nombre"].values[0]
        st.session_state.carrito_compra.append({
            "producto_id": int(prod_id), "nombre": nombre, "cantidad": cantidad,
            "costo": costo, "subtotal": cantidad * costo
        })
        st.success("Agregado")
        st.rerun()
...
def pagina_gastos():
    ...
    if st.button("💾 Guardar gasto"):
        try:
            with get_connection() as conn:
                c = conn.cursor()
                c.execute("INSERT INTO gastos (fecha, categoria, descripcion, monto, metodo_pago) VALUES (%s, %s, %s, %s, %s)",
...
def pagina_historial():
    ...
    with get_connection() as conn:
        with tab1:
            df = pd.read_sql_query("SELECT id, fecha, cliente, total, descuento, metodo_pago FROM ventas WHERE anulada = 0 ORDER BY fecha DESC LIMIT 50", conn)
            st.dataframe(df, use_container_width=True, hide_index=True) if not df.empty else st.info("Sin ventas")
        with tab2:
            df = pd.read_sql_query("SELECT id, fecha, proveedor, total, metodo_pago FROM compras WHERE anulada = 0 ORDER BY fecha DESC LIMIT 50", conn)
            st.dataframe(df, use_container_width=True, hide_index=True) if not df.empty else st.info("Sin compras")
        with tab3:
            df = pd.read_sql_query("SELECT fecha, categoria, descripcion, monto FROM gastos ORDER BY fecha DESC LIMIT 50", conn)
    ...
    with tab4:
        ...
        with get_connection() as conn:
            ventas = pd.read_sql_query("SELECT id, fecha, cliente, total FROM ventas WHERE anulada = 0 ORDER BY fecha DESC LIMIT 30", conn)
        if ventas.empty:
            st.warning("No hay ventas para anular")
        else:
            venta_id = st.selectbox("Selecciona la venta", options=ventas["id"].tolist(),
    ...
    with tab5:
        ...
        with get_connection() as conn:
            compras = pd.read_sql_query("SELECT id, fecha, proveedor, total FROM compras WHERE anulada = 0 ORDER BY fecha DESC LIMIT 30", conn)
        if compras.empty:
            st.warning("No hay compras para anular")
        else:
            compra_id = st.selectbox("Selecciona la compra", options=compras["id"].tolist(),
                        ...
                        with get_connection() as conn:
                            ...
                            for d in detalles:
                                ...
                                if c.fetchone()["stock"] < d["cantidad"]:
                                    st.error("Stock insuficiente para anular esta compra")
                                    return
...
def main():
    ...
    pagina = st.sidebar.radio("Menú", [
        "📊 Dashboard", "📦 Inventario", "🛒 Ventas", "📥 Compras",
